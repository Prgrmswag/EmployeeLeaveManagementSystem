...
if(isset($_POST['update']))
{
    ...
    $q1 = $dbh->prepare("SELECT tblleaves.LeaveType, tblleaves.empid, tblleaves.FromDate, tblleaves.ToDate FROM tblleaves WHERE tblleaves.id='$did'");
    $q1->execute();
    $r2 = $q1->fetchAll(PDO::FETCH_OBJ);

    $employeeId = $r2[0]->empid;
    $leaveType = $r2[0]->LeaveType;
    $leaveType = str_replace(' ','',$leaveType);

    $q2 = $dbh->prepare("SELECT totalleaves.$leaveType FROM totalleaves WHERE totalleaves.empId='$employeeId'");
    $q2->execute();
    $r3 = $q2->fetchAll(PDO::FETCH_OBJ);

    $value = (int)$r3[0]->$leaveType;
    $StartDateDay = explode("/",$r2[0]->FromDate);
    $EndDateDay = explode("/",$r2[0]->ToDate);    
    $StartDateMonth = $StartDateDay[1];
    $EndDateMonth = $EndDateDay[1];
    $StartDateYear = $StartDateDay[2];
    $EndDateYear = $EndDateDay[2];
    $TotalDaysForStartMonth = date('t',strtotime($r2[0]->FromDate));

    if(abs($StartDateMonth[1] - $EndDateMonth[1]) == 1){
        $DaysInCurrentMonth = $TotalDaysForStartMonth - $StartDateDay[0];
        $subtractBy = $DaysInCurrentMonth + $EndDateDay[0];
        $totalValue = $value - $subtractBy; 
    }
    else if(abs($StartDateMonth[1] - $EndDateMonth[1]) > 1){
        $subtractBy = 100;
    }
    else{
        $subtractBy = abs($EndDateDay[0]-$StartDateDay[0]);
        $totalValue = $value - $subtractBy;
    }
    ...
    if($value >= $subtractBy){
        ...
        if($status == 1){
            foreach($r3 as $res){
                $q3=$dbh->prepare("UPDATE totalleaves SET $leaveType=$totalValue WHERE totalleaves.empId='$employeeId'");
                $q3->execute();
            }
        }
    }
        ...
        if($status == 2){
            $admremarkdate=date('Y-m-d G:i:s ', strtotime("now"));
            $sql="update tblleaves set Status=:status, AdminRemark=:description, AdminRemarkDate=:admremarkdate where id=:did";
            $query = $dbh->prepare($sql);
            $query->bindParam(':status',$status,PDO::PARAM_STR);
            $query->bindParam(':did',$did,PDO::PARAM_STR);
            $query->bindParam(':description',$description,PDO::PARAM_STR);
            ...
            $error="Available leaves are not sufficient for this leave to be approved";
        }
    }
