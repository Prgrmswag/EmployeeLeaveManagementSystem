...
$cnt=1;
$sql2 = "SELECT COLUMN_NAME as name FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = N'totalleaves';";
...
if($stats==1){
 ...
 <?php } ?>
                                             ...
                                             <td><?php foreach($results2 as $result2){
                                                if($result2->name != 'empId')
                                                {
                                                    $column = preg_split("/(?=(?<![A-Z]|^)[A-Z])/",$result2->name);
                                                    ...
                                                    $q2=$dbh->prepare("SELECT * FROM totalleaves WHERE totalleaves.empId='$result->id'");
                                                    $q2->execute();
                                                    $r2 = $q2->fetchAll(PDO::FETCH_OBJ);
                                                    foreach($r2 as $res){
                                                        $columnname = $result2->name;
                                                        $value = $res->$columnname;
                                                        if($result->LeaveType === $tempname){
