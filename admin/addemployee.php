...
$sql2="INSERT INTO totalleaves (empId) VALUES($lastInsertId)";
$query2 = $dbh->prepare($sql2);
$query2->execute();

$q = $dbh->prepare("SELECT COLUMN_NAME as name FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = N'totalleaves';");
$q->execute();
...
foreach($r as $res){
   if($res->name != 'empId')
    {
        $columnname = '';
        $column = preg_split("/(?=(?<![A-Z]|^)[A-Z])/",$res->name);
        foreach($column as $col){
            $columnname .= $col . ' ';
        }
        
        $q2=$dbh->prepare("SELECT * FROM tblleavetype WHERE tblleavetype.LeaveType='$columnname'");
        $q2->execute();
        $r2 = $q2->fetchAll(PDO::FETCH_OBJ);
        foreach($r2 as $res2){
            $value = (int)$res2->TotalLeaves;
            $q3 = $dbh->prepare("UPDATE totalleaves SET $res->name=$value WHERE totalleaves.empId=$lastInsertId");
...
function checkPhoneNumber(){
    ...
    $isNotValid = true;
    jQuery.ajax({
        url: "phonenumbervalidation.php",
        data: {phone: $("#phone").val()},
        type: "POST",
        success:function(data){
        $("#phone-validity").html(data);
        $("#loaderIcon").hide();
        },
        error:function (){}
    });
...
<input id="phone" name="mobileno" type="text" onBlur=checkPhoneNumber() maxlength="10" autocomplete="off" required>
<span id="phone-validity" style="font-size:12px;"></span>
 </div>
